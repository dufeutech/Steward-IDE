...
An interactive session backed by an operating-system shell process — its lifecycle, byte-transparent input and output, viewport-size tracking, exit reporting, and the boundary that keeps it inside the application's lifetime and privileges.
