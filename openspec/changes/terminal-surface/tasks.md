...
- [ ] 6.3 Publish one signed release containing both `xkin` and `terminal` so no client sees a tree with one and not the other (design, Migration Plan step 2)
...
- [ ] 7.3 Run the application and verify by hand what only a running app can show: a full-screen program redraws in place, the interrupt chord reaches a running command, a resize reflows it, double-width text stays aligned, and a dense build's output does not stall the window
