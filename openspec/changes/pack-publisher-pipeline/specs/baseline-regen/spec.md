...
If the fetched payload does not hash-match the committed baseline manifest, the tool
MUST refuse it and report exactly which files mismatch. Updating the committed manifest
MUST be a separate, explicit operation — never a side effect of regeneration.
