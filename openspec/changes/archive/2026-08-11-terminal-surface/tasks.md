...
- [x] 6.3 **Published — metadata v7, run `31462672658`, 30 s.** Dispatched against `change/terminal-interrupt-signal` rather than `main`: the workflow builds and verifies against the ref's own committed manifest and anchor, so the repository is still the source of truth about what is live, and the branch merges to `main` in the same session. The first-party path ran as designed — payload built, verified against the committed manifest, signed, attested, overlaid onto the live tree and re-verified with `tuftool download` before the push. The fix below held: **the tree still carries `xkin`'s blobs**, 105 targets against the 102 that were there before. The original defect it guards against: `tuftool update` writes only the targets a release adds while signing metadata that names every target of every release, so the workflow's `rm -rf` of the published tree would have deleted `xkin`'s 101 blobs behind metadata still pinning them. Measured locally (3 blobs written, 105 named) and reproduced as a client failure. The workflow now overlays and verifies the assembled tree with `tuftool download` before pushing
- [x] 6.4 **Verified against what Pages actually serves**, `tuftool 0.17.0` against the committed anchor and the two public URLs, into a directory that did not exist: exit zero, 105 files, and **both** `terminal.manifest.json` and `xkin.manifest.json` present — which is the property 6.3 is really about. A conforming client walked the root chain, checked expiry and version monotonicity, and hash-verified every target it wrote

## 7. Cutover and verification

- [x] 7.1 **Cut over.** `terminal` is listed as a second `application` pack, after `xkin` and before `bootstrap`, so the composed page carries the framework first. Safe in this order only because 6.4 proved the pack is already served: a client that reads this config finds the pack waiting for it
...
- [x] 7.3 Run the application and verify by hand what only a running app can show. Done against a local two-pack endpoint; four of five held here, and the fifth is fixed by the follow-up change `terminal-interrupt-signal` (design D4c):
  ...
  - [x] **the interrupt chord reaches a running command.** It did not when measured here — Ctrl+C cancelled the shell's prompt line but left `ping -t` and `timeout /t` running under both shells. Measured down to the adapter and against Windows Terminal as a control (design D4c), then fixed in `terminal-interrupt-signal`: the interrupt is raised as a control event on the session's console, and the shell now answers in 52.7 ms
...
- [x] 9.3 Archived to `openspec/changes/archive/2026-08-11-terminal-surface/`, after the specs were synced and the release was live and verified
