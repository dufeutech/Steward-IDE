...
The binary carries no application content. It embeds one small first-party pack — the
bootstrap recovery surface — and the application pack is acquired once, on first run.
Selection between them is a pure decision in the core, so resolution has no
baseline-specific branch: a pack that the binary does not embed simply contributes no
fallback candidate and resolves to nothing, which is not a fault.
...
flowchart TB
    ...
    subgraph corebox [Core — pure, no I/O]
        ...
        compose["shell: compose = application packs\nif all resolve, else bootstrap"]
        plan["updater: plan = missing blobs only"]
        progress["updater: progress = outstanding bytes"]
    end
    ...
    compose --> tags
    resolve --> cas
    resolve -. "fallback: active → previous" .-> cas
    compose -. "no application version yet,\nor none resolvable" .-> bootstrap
    tags --> manifest
    updater --> plan
    updater --> progress
    updater --> verify
    updater --> active
    updater --> pending
    progress -. "acquisition_progressed / _failed" .-> shell
    tuf --> endpoint
    updater --> tuf
```
...
flowchart TB
    ...
    pinned["app/packs/&lt;pack&gt;/manifest.json\nthe pin — publisher input,\nnot bundled in the binary"]
    bootstrapdir["packs-baseline/bootstrap/\nfirst-party, no external origin,\nmanifest by the same generator"]
...
Because the binary embeds no application content, this endpoint is now what makes a new
install useful, not only what keeps it current. A fresh install with the endpoint
unreachable renders the bootstrap surface, reports why, and offers a retry — legible, but
it is application content that is missing until the endpoint answers. Existing installs
are unaffected: they serve what their store already holds.
